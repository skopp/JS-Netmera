...
/**
 * @class NetmeraGeoLocation is used to create location with the given latitude
 *        and longitude values. It is used to set location into the content and
 *        then use it on the search queries.
 * @name NetmeraGeoLocation
 * @example The example below shows how it is used. var location = new
 *          NetmeraGeoLocation(41,29); var content = new
 *          NetmeraContent("SampleContent"); content.add("location", location);
...
/**
 * @class NetmeraUser object is for managing users of the application. You can
...
/**
 * @class Exception handling mechanism for Netmera API. It throws an exception
 *        when there is a failure while creating,editing,deleting and searching
 *        data. Also it throws and exception when there is an network connection
 *        error.
 * @name NetmeraException
 * @description Creates an exception
 * @param {Integer}
 *            errorCode Code of this exception
 * @param {String}
 ...
 * @param file
 *            file object
 * @example // &lt;input type="file" id="fileInput" /&gt; var file =
 *          document.getElementById("fileInput").files[0]; var media = new
 *          NetmeraMedia(file); var content = new NetmeraContent("Blog");
 *          content.add("title", "My Blog Title"); content.add("image", media);
 *          content.create();
 */
var NetmeraMedia;
...
/**
 * @class NetmeraPush class is used to send push notifications into ANDROID and
 *        IOS devices from javascript code.
 * @augments BasePush
 * @name NetmeraPush
 * @description Constructor that creates a new push notification object to send
 *              push notifications to IOS or Android devices.
 * @example var push = new NetmeraPush(); push.setSendToIos(true);
 *          push.setSendToAndroid(true); push.setAppName("12345");
 *          push.setMessage("This is the message that will be sended as a
 *          notification"); push.sendNotification();
 */
var NetmeraPush;
 ...
 * @example var iosPush = new NetmeraIOSPush();
 * 			iosPush.setDeviceGroups(["News", "Sports", "Media"]);
 * 			iosPush.setMessage("This is an example push message to ios devices!");
 * 			iosPush.sendNotification(function() {
 * 				// On success.
 * 			}, function(error) {
 ...
 * @example var androidPush = new NetmeraIOSPush();
 * 			androidPush.setDeviceGroups(["News", "Sports", "Media"]);
 * 			androidPush.setMessage("This is an example push message to android devices!");
 * 			androidPush.sendNotification(function() {
 * 				// On success.
 * 			}, function(error) {
 * 				// When any error occured
 * 			});
...
(function () {
    ...
    function send(params, successCB, errorCB) {
        ...
        if (params.content)
            req.url = req.url + "&" + serialize(params.data) + "&content=" + encodeURIComponent(JSON.stringify(params.content));
        else
    ...
    function getAlbumData(content, actionToken, file, onsuccess, onfail) {
    	...
    	params.data = {};
    	params.data["opensocial_viewer_id"] = content.owner.nodeName;
    	params.data["contentPath"] = content.content.path;
    	params.data["opensocial_netmera_domain"] = _request.domain;
    	params.data["opensocial_app_id"] = content.site;
    	params.method = _request.get;
    ...
    function getUploadUrl(content, path, uploadKey, site, domain, onsuccess, onfail) {
    	...
    	params.data = {};
    	params.data["album"] = path;
    	params.data["uploadedPhotoHash"] = uploadKey;
    	params.data["cdnDomain"] = _request.url + "/cdn";
    	params.data["opensocial_app_id"] = site;
    	params.data["opensocial_netmera_domain"] = domain;
    	params.data["opensocial_viewer_id"] = content.owner.nodeName;
    ...
    NetmeraService = function (objectName) {
		 ...
		 * @example You can use NetmeraService.SortOrder to set sort order var
		 *          service = new NetmeraService("__CONTENT_TYPE_NAME__");
        ...
        /**
		 * Sets the current page of items to return. If it is less than 0 then
		 * it is setted to 0.
		 * 
		 * @param {Integer}
		 *            page the current page of items to return
		 * @example For example, if you set max as 10 and set page as 3, search
		 *          results start with the 30. content
		 */
		 ...
		 * @example var service = new NetmeraService("ContentName");
		 *          service.count(function(num) { // Number of contents is 'num' },
		 *          function(error) { // Operation failed, look at 'error' to
		 *          identify the error });
		 */
        ...
        /**
		 * Retrieves the Array of NetmeraContent objects that matches with the
		 * query.
		 * 
		 * @param {Function}
		 *            onsuccess callback function called when search is
		 *            successful
		 * @param {Function}
		 *            onfail callback function called when search failed
		 * @example var service = new NetmeraService("ContentName");
		 *          service.search(function(entries) { // iterate through
		 *          'entries' array }, function(error) { // Do something with
		 *          'error' });
		 */
        ...
        /**
		 * Creates box using the given two location (latitude,longitude) data
		 * and searches inside that box.
		 * 
		 * @param {NetmeraGeoLocation}
		 *            firstGeoPoint NetmeraGeoLocation object, which is the
		 *            corner of the box
		 * @param {NetmeraGeoLocation}
		 *            secondGeoPoint NetmeraGeoLocation object, which the other
		 *            corner of the box
		 * @param {String}
		 *            locationField name of the field that holds location data.
		 * @param {Function}
		 *            onsuccess callback function called when boxSearch is
		 *            successful
		 * @param {Function}
		 *            onfail callback function called when boxSearch failed
		 * @example var service = new NetmeraService("ContentName"); var corner1 =
		 *          new NetmeraGeoLocation(40.7, 28.6); var corner2 = new
		 *          NetmeraGeoLocation(41.2, 29.4); service.boxSearch(corner1,
		 *          corner2, "myLocationField", function(entries) { //
		 *          entries[0], entries[1], ...., entries[entries.length-1] },
		 *          function(error) { // Identify 'error' with error.getCode()
		 *          });
		 */
        ...
        /**
		 * Searches the content by taking given location as a base and retrieves
		 * the contents that located given distance far away.
		 * 
		 * @param {NetmeraGeoLocation}
		 *            geoPoint base location to search near it.
		 * @param {Number}
		 *            distance distance used to create circle by taking the
		 *            startLocation as a center.
		 * @param {String}
		 *            locationField name of the field that holds location data.
		 * @param {Function}
		 *            onsuccess callback function called when circleSearch is
		 *            successful
		 * @param {Function}
		 *            onfail callback function called when circleSearch failed
		 * @example var service = new NetmeraService("ContentName"); var center =
		 *          new NetmeraGeoLocation(41, 29); var distance = 4; // 4
		 *          kilometers service.circleSearch(center, distance,
		 *          "myLocationField", function(entries) { // entries[0],
		 *          entries[1], ...., entries[entries.length-1] },
		 *          function(error) { // Identify 'error' with error.getCode()
		 *          });
		 */
        ...
        /**
		 * Gets the single NetmeraContent object whose path is equal to the
		 * given path with setPath(path).
		 * 
		 * @param {Function}
		 *            onsuccess callback function called when get is successful
		 * @param {Function}
		 *            onfail callback function called when get method failed
		 * @example var service = new NetmeraService("ContentName");
		 *          service.setPath("Your Content's Path");
		 *          service.get(function(content) { var path =
		 *          content.getPath(); }, function(error) { // Identify 'error'
		 *          with error.getCode() });
		 */
        ...
        /**
		 * Retrieves the Array of NetmeraUser objects that matches with the
		 * query.
		 * 
		 * @param {Function}
		 *            onsuccess callback function called when searchUser is
		 *            successful
		 * @param {Function}
		 *            onfail callback function called when searchUser failed
		 * @example var service = NetmeraService.getNetmeraUserService();
		 *          service.searchUser(function(users) { for(var i=0; i <
		 *          users.length; i++) { console.log(users[i].getNickname()); } },
		 *          function(error) { console.log(error.getMessage()); });
		 */
    ...
    NetmeraMedia = function (mediaUrl) {
		 ...
		 * 
		 * @param {NetmeraMedia.PhotoSize}
		 *            size size of the image
		 * @return {String} the URL of file with the given size.
		 * @example var service = new NetmeraService("Blog");
		 *          service.setPath("/mobimeracontents/_34");
		 *          service.get(function(content) { var media =
		 *          content.getNetmeraMedia("image"); var url =
		 *          media.getUrl(NetmeraMedia.PhotoSize.THUMBNAIL); // do
		 *          something with the url }, function(error) { // handle
		 *          exception });
		 */
    ...
    NetmeraMedia.PhotoSize = {
        /**
		 * @constant
		 * @description This is the original image size.If no photo size is
		 *              setted then original size is used.
		 */
        DEFAULT: '/org',
        /**
		 * @constant
    ...
    function sendPush(message, deviceGroups, types, onsuccess, onfail) {
    	if (!message) {
    ...
    NetmeraPush = function() {
    	...
    	this.sendNotification = function(onsuccess, onfail) {
    		...
    		sendPush(this.getMessage(), this.getDeviceGroups(), types, onsuccess, onfail);
    	};
    };
    ...
    function registerAndLogin(fbId, nickname, name, surname, email, onsuccess, onfail) {
        ...
        params.method = _request.post;
        params.data = {};
        params.data["method"] = _request.facebookLoginMethod;
        params.data.params = {};
    ...
    NetmeraFacebookUtils.initialize = function(params) {
    	if (!FB || !FB.init) {
    		console.log("TODO: error callback");
    		return;
    	}
    ...
    NetmeraFacebookUtils.login = function(permissions, onsuccess, onfail) {
    	facebookGetMe(permissions, function(response) {
    		...
    		if (permissions instanceof Array && permissions.indexOf("email") != -1) {
    			askForEmail = true;
    		}
    ...
    var _consumerKey = "";
    var _consumerSecret = "";
    var _netmeraCallbackUrl = _request.url + "/mobimera/callbacks/twittercallback.html";
    var _twitterRequestTokenUrl = "https://api.twitter.com/oauth/request_token";
    var _twitterAuthenticateUrl = "https://api.twitter.com/oauth/authenticate?oauth_token=";
    var _twitterAccessTokenUrl = "https://api.twitter.com/oauth/access_token";
    var _twitterVerifyCredentialsUrl = "https://api.twitter.com/1/account/verify_credentials.json?skip_status=true";
    var _twitter = null;
    ...
    function createOAuthHeaders(url, params, accessor, method) {
    	...
    	if (params.oauth_callback) {
    		message.parameters.push(["oauth_callback", params.oauth_callback]);
    	}
    	
    	if (params.oauth_token) {
    		message.parameters.push(["oauth_token", params.oauth_token]);
    	}
    	
    	if (params.oauth_verifier) {
    		message.parameters.push(["oauth_verifier", params.oauth_verifier]);
    	}
    	
    	if (params.oauth_version) {
    		message.parameters.push(["oauth_version", params.oauth_version]);
    	}
    ...
    function getRequestToken(onsuccess, onfail) {
    	var params = {
    		"oauth_callback": _netmeraCallbackUrl
    	};
    	var accessor = getAccessor(_consumerSecret, "");
      	var oauthHeaders = createOAuthHeaders(_twitterRequestTokenUrl, params, accessor, _request.post);
      	var requestParams = {};
      	requestParams.url = _twitterRequestTokenUrl;
    ...
    function getActionToken(oauthToken, oauthVerifier, onsuccess, onfail) {
    	...
    	var accessor = getAccessor(_consumerSecret, "");
    	var oauthHeaders = createOAuthHeaders(_twitterAccessTokenUrl, params, accessor, _request.post);
    	var requestParams = {};
    	requestParams.url = _twitterAccessTokenUrl;
    	requestParams.method = _request.post;
    	requestParams[_request.params.oauthHeaders] = oauthHeaders;
    	sendSigned(requestParams, function(resp) {
    ...
    function getTwitterUserData(onsuccess, onfail) {
    	if (_twitter) {
    		var params = {
				"oauth_token": _twitter.getAccessToken(),
				"oauth_version": "1.0"
    		};
    		var accessor = getAccessor(_consumerSecret, _twitter.getSecretToken());
    		var oauthHeaders = createOAuthHeaders(_twitterVerifyCredentialsUrl, params, accessor, _request.get);
    		var requestParams = {};
        	requestParams.url = _twitterVerifyCredentialsUrl;
        	requestParams.method = _request.get;
        	requestParams[_request.params.oauthHeaders] = oauthHeaders;
        	sendSigned(requestParams, function(response) {
        		var resp = JSON.parse(response);
        		var nickname = "", name = "", surname = "", userid = "";
        		if (resp["id"] && resp["screen_name"]) {
        			userid = resp["id"];
    ...
    function twitterRegisterAndLogin(nickname, userid, name, surname, onsuccess, onfail) {
        ...
        params.method = _request.post;
        params.data = {};
        params.data["method"] = _request.twitterLoginMethod;
        params.data.params = {};
    ...
    NetmeraTwitterUtils.login = function(onsuccess, onfail) {
    	getRequestToken(function(requestToken) {
    		twitterPopup(_twitterAuthenticateUrl + requestToken, function(oauthToken, oauthVerifier) {
